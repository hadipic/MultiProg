...
//      [11:8] 17th address bit location (added to control byte)
{
    ...
    unsigned int start=CommonFunctions::CommonFunctions::GetTickCount();
    hvreg=0;
    usb->bufferU[0]=0;
    j=1;
    ...
    //    WriteLogIO();
    //****************** usb->read ********************
    CommonFunctions::printMessage(GlobalVariables::strings[S_ReadEE]);		//usb->read EEPROM ...
    //////PrintStatusSetup();
    int inc=0;
        ...
        /*if(usb->saveLog)
{
        ...
        emit PrintStatus(GlobalVariables::strings[S_CodeReading2],i*100/(dim),i);	//"usb->read: %d%%, addr. %05X"
    ...
    if(k!=dim)
    {
        ...
        CommonFunctions::printMessage2(GlobalVariables::strings[S_ReadEEErr],dim,k);	//"Error reading EEPROM area, requested %d bytes, usb->read %d\r\n"
        usb->sizeEE=k;
    }
    ...
    /*if(usb->saveLog)
{
    ...
    //****************** usb->Write ********************
    CommonFunctions::printMessage(GlobalVariables::strings[S_EEAreaW]);	//"usb->Write EEPROM ... "
    ////PrintStatusSetup();
    ...
    for(i=0,j=1;i<dim;i+=page)
    {
        ...
        j=1;
        emit PrintStatus(GlobalVariables::strings[S_CodeWriting2],i*100/(dim),i);	//"usb->Write: %d%%, addr. %04X"
        emit onProgressChanged(i, dim);
        /*if(usb->saveLog)
{
            fprintf(logfile,GlobalVariables::strings[S_Log7],i,i,k,k);	//"i=%d(0x%X), k=%d(0x%X)\n"
    ...
    CommonFunctions::printMessage(GlobalVariables::strings[S_EEV]);	//"Verify EEPROM ... "
    ////PrintStatusSetup();
    k=0;
    int inc;
    for(i=0,j=1;i<dim;i+=inc)
    {
        ...
        if((usb->bufferI[1]==I2C_READ||usb->bufferI[1]==I2C_READ2)&&usb->bufferI[2]<0xFA)
        {
            for(z=3;z<usb->bufferI[2]+3&&z<usb->DIMBUF;z++)
            {
                ...
                {
                    CommonFunctions::printMessage("\r\n");
                    CommonFunctions::printMessage4(GlobalVariables::strings[S_CodeVError],i+z-3,i+z-3,usb->memEE[k-1],usb->bufferI[z]);	//"Error verifying address %04X (%d), written %02X, usb->read %02X\r\n"
                    err++;
                }
            }
        }
        emit PrintStatus(GlobalVariables::strings[S_CodeV2],i*100/(dim),i);	//"Verify: %d%%, addr. %04X"
        j=1;
        emit onProgressChanged(i, dim);
        /*if(usb->saveLog)
{
    ...
    /*if(usb->saveLog)
{
    ...
    for(i=0;i<dim2;)
    {
        ...
        emit PrintStatus(GlobalVariables::strings[S_CodeReading2],i*100/dim2,i);	//"usb->read: %d%%, addr. %05X"
        j=1;
        /*if(usb->saveLog)
{
            fprintf(logfile,GlobalVariables::strings[S_Log7],i,i,k,k);	//"i=%d(0x%X), k=%d(0x%X) \n"
    ...
    for(i=0,j=1;i<dim;i+=n)
    {
        ...
        if(usb->bufferI[1]==uWRX&&usb->bufferI[2]<0xFA)
        {
            for(z=3;z<usb->bufferI[2]/8+3&&z<usb->DIMBUF;z+=2,k+=2)
            {
                ...
                {
                    CommonFunctions::printMessage("\r\n");
                    CommonFunctions::printMessage4(GlobalVariables::strings[S_CodeVError],i+z-3,i+z-3,usb->memEE[k],usb->bufferI[z+1]);	//"Error verifying address %04X (%d), written %02X, usb->read %02X\r\n"
                    err++;
                }
            }
        }
...
// options=0: x16 organization     =1: x8 organization
{
    ...
    if(i!=dim)
    {
        CommonFunctions::printMessage2(GlobalVariables::strings[S_CodeWError4],i,dim);	//"Error writing code area, requested %d bytes, usb->read %d\r\n"
    }
    ...
    for(i=0;i<dim2;)
    {
        ...
        for(z=1;z<usb->DIMBUF-3;z++)
        {
            ...
            if(usb->bufferI[z]==uWRX)
            {
                ...
                {		//x8
                    ...
                    {
                        CommonFunctions::printMessage("\r\n");
                        CommonFunctions::printMessage4(GlobalVariables::strings[S_CodeVError],k,k,usb->memEE[k],usb->bufferI[z+2]);	//"Error verifying address %04X (%d), written %02X, usb->read %02X\r\n"
                        err++;
                    }
                    k++;
                }
                    ...
                    {
                        CommonFunctions::printMessage("\r\n");
                        CommonFunctions::printMessage4(GlobalVariables::strings[S_CodeVError],k,k,usb->memEE[k],usb->bufferI[z+3]);	//"Error verifying address %04X (%d), written %02X, usb->read %02X\r\n"
                        err++;
                    }
                    if(usb->memEE[k+1]!=usb->bufferI[z+2])
                    {
                        CommonFunctions::printMessage("\r\n");
                        CommonFunctions::printMessage4(GlobalVariables::strings[S_CodeVError],k+1,k+1,usb->memEE[k+1],usb->bufferI[z+2]);	//"Error verifying address %04X (%d), written %02X, usb->read %02X\r\n"
                        err++;
                    }
                    k+=2;
                }
        ...
        emit PrintStatus(GlobalVariables::strings[S_CodeV2],i*100/dim2,i);	//"Verify: %d%%, addr. %04X"
        j=1;
        if(usb->saveLog)
        {
            //fprintf(logfile,GlobalVariables::strings[S_Log8],i,i,k,k,err);	//"i=%d, k=%d, err=%d\n"
...
// dim=size in bytes
{
    ...
    //if(ID>0&&ID!=0xFFFFFF)
    CommonFunctions::printMessage1("DEVICE ID=0x%06X\r\n",ID);
    //****************** usb->read ********************
    CommonFunctions::printMessage(GlobalVariables::strings[S_ReadEE]);		//usb->read EEPROM ...
    ////PrintStatusSetup();
...
// automatic usb->Write delay
{
    ...
    for(z=1;z<usb->DIMBUF-4&&usb->bufferI[z]!=SPI_READ;z++);
    ID=(usb->bufferI[z+2]<<16)+(usb->bufferI[z+3]<<8)+usb->bufferI[z+4];
    //if(ID>0&&ID!=0xFFFFFF) 	//CommonFunctions::printMessage1("DEVICE ID=0x%06X\r\n",ID);
    if(options&0x1000)
...
void ProgEEPROM::OW_ID(int id)
{
    ...
    for(i=0;i<sizeof(OW_LIST)/sizeof(OW_LIST[0]);i++)
    {
        if(id==OW_LIST[i].id)
        {
            ...
            CommonFunctions::printMessage(s);
            return;
        }
    }
    //sprintf(s,"%s",GlobalVariables::strings[S_nodev]); //"Unknown device\r\n");
    CommonFunctions::printMessage(s);
}
...
//          2=protection bytes + ID after memory area
{
    ...
    {	//no presence pulse
        CommonFunctions::printMessage(GlobalVariables::strings[S_ComErr]);		//communication error
    ...
    if(z<usb->DIMBUF-9)
    {
        CommonFunctions::printMessage1("Family code: 0x%02X ",usb->bufferI[z+2]);
        OW_ID(usb->bufferI[z+2]);
        CommonFunctions::printMessage3("Serial ID: 0x%02X%02X%02X",usb->bufferI[z+3],usb->bufferI[z+4],usb->bufferI[z+5]);
        CommonFunctions::printMessage3("%02X%02X%02X",usb->bufferI[z+6],usb->bufferI[z+7],usb->bufferI[z+8]);
        CommonFunctions::printMessage1("\r\nCRC: 0x%02X\r\n",usb->bufferI[z+9]);
    }
    //****************** usb->read ********************
    ////PrintStatusSetup();
    for(i=0,j=1;i<dim;i+=usb->DIMBUF-4)
    ...
    {		//usb->read status register + application register
        ...
        //if(z<usb->DIMBUF-2) CommonFunctions::printMessage1("Status register: 0x%02X\r\n",usb->bufferI[z+2]);
        for(z+=2;usb->bufferI[z]!=OW_READ&&z<usb->DIMBUF-10;z++);
        CommonFunctions::printMessage("Application register: 0x");
        //for(i=z+2;i<z+10&&i<usb->DIMBUF;i++)
        CommonFunctions::printMessage1("%02X",usb->bufferI[i]);
        CommonFunctions::printMessage("\r\n");
    }
    else if(options==2)
    {		//usb->read protection & ID bytes
        ...
        if(usb->bufferI[1]==OW_READ&&usb->bufferI[2]<0xFA)
        {
            if(dim==0xA00)
            {
                //for(i=0;i<10;i++)  CommonFunctions::printMessage2("Protection Control Byte Block %d: 0x%02X\r\n",i,usb->bufferI[i+3]);
                CommonFunctions::printMessage("User EEPROM:\r\n");
                //for(i=0;i<10;i++) CommonFunctions::printMessage1("%02X",usb->bufferI[i+13]);
                CommonFunctions::printMessage("\r\n");
                //for(i=0;i<10;i++) CommonFunctions::printMessage1("%02X",usb->bufferI[i+23]);
                CommonFunctions::printMessage1("\r\nMemory Block Lock: 0x%02X\r\n",usb->bufferI[33]);
                CommonFunctions::printMessage1("Register Page Lock: 0x%02X\r\n",usb->bufferI[34]);
                CommonFunctions::printMessage1("Factory Byte: 0x%02X\r\n",usb->bufferI[35]);
                CommonFunctions::printMessage2("Factory Trim Bytes: 0x%02X%02X\r\n",usb->bufferI[36],usb->bufferI[37]);
                CommonFunctions::printMessage2("Manufacturer ID: 0x%02X%02X\r\n",usb->bufferI[38],usb->bufferI[39]);
            }
            else{
                CommonFunctions::printMessage1("Protection Control Byte Page 0: 0x%02X\r\n",usb->bufferI[3]);
                CommonFunctions::printMessage1("Protection Control Byte Page 1: 0x%02X\r\n",usb->bufferI[4]);
                CommonFunctions::printMessage1("Protection Control Byte Page 2: 0x%02X\r\n",usb->bufferI[5]);
                CommonFunctions::printMessage1("Protection Control Byte Page 3: 0x%02X\r\n",usb->bufferI[6]);
                CommonFunctions::printMessage1("Copy Protection Byte: 0x%02X\r\n",usb->bufferI[7]);
                CommonFunctions::printMessage1("Factory Byte: 0x%02X\r\n",usb->bufferI[8]);
                CommonFunctions::printMessage2("User Bytes/Manufacturer ID: 0x%02X%02X\r\n",usb->bufferI[9],usb->bufferI[10]);
            }
        }
    }
    //PrintStatusEnd();
...
//          1 = 32 byte scratchpad
{
    ...
    CommonFunctions::printMessage(GlobalVariables::strings[S_Compl]);	//"completed\r\n"
    //****************** verify EEPROM ********************
    CommonFunctions::printMessage(GlobalVariables::strings[S_EEV]);	//"Verify EEPROM ... "44
    ////PrintStatusSetup();
    k=0;
    usb->bufferU[j++]=OW_RESET;
    usb->bufferU[j++]=OW_WRITE;
    ...
    if(usb->saveLog)
    {
        //fprintf(logfile,GlobalVariables::strings[S_EEV]);	//"Verify EEPROM ... "
        //WriteLogIO();
    }
    for(i=0,j=1;i<dim;i+=usb->DIMBUF-4)
    {
...
// usb->read DS1820 one-wire digital thermometer
{
    ...
    //CommonFunctions::printMessage("0x");
    //for(i=z+2;i<z+10&&i<usb->DIMBUF;i++) CommonFunctions::printMessage1("%02X",usb->bufferI[i]);
    //CommonFunctions::printMessage("\r\n");
    int T=usb->bufferI[z+2]+(usb->bufferI[z+3]<<8);
    if(T>0xF000) T|=0xFFFF0000; //adjust negative value
#ifdef _MSC_VER
    CommonFunctions::printMessage2("T=%.4f°C  (0x%04X)\r\n",T*TLSB,T);
#else
    CommonFunctions::printMessage2("T=%.4fÂ°C  (0x%04X)\r\n",T*TLSB,T);
#endif
    //****************** exit ********************
    usb->bufferU[j++]=EN_VPP_VCC;		//turn off
    usb->bufferU[j++]=0x0;
