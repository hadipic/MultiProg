...
public class AndroidUSBTools extends QtActivity
{
    ...
    public String openDevice()
    {
		...
		readEndpoint = intf.getEndpoint(0);
    ...
    public static String SendData(byte[] bytes)
    {
        if(m_instance  == null)
        {
            m_instance = new AndroidUSBTools();
        ...
        return m_instance.sendData(bytes);
    }

    public String sendData(byte[] bytes)
    {
        if (openSuccess)
        {
            int TIMEOUT = 1;
    ...
    private String readData()
    {
        ...
        if (openSuccess)
        {
            int packetSize = 64;//readEndpoint.getMaxPacketSize();
            //String result = "PacketSize:"+Integer.toString(packetSize)+"\n";

            byte[] bytes = new byte[packetSize];
						
            int TIMEOUT = 0;
            int r = connection.bulkTransfer(readEndpoint, bytes, bytes.length, TIMEOUT);
            ...
            if (r >= 0)
			{
				...
				{
					trancatedBytes[i] = b;
                    result+=Integer.toString(b & 0xFF)+" ";
					i++;
				}
                //return trancatedBytes;
            }
            //result+=" r:"+Integer.toString(r);
        }
        return result;
    }
